...
#define ZERO 0 // like a search and replace tool, not very encouraged to use in C++
...
namespace chap3 {
	void chap3Study() {
		...
		const int MONTHS = 12; // must be initialized when declared; cannot initialize later
		...
		std::cout << 5 / -2 << std::endl;
		std::cout << 5 % -2 << std::endl;
		std::cout << 5 / 2.0 << std::endl; // result is a float number

		// datatype conversion
		char cha = 34; // allowed
		int abc = 34.56; // 34
		char cha2 = { 34 }; // allowed since 34 can fit in char
		// char cha3 = { abc }; // not allowed since a can be changed into a bigger value
		char cha4 = 31325; // 93, discard bits bigger than 2^8
		std::cout << cha4 << std::endl;
		std::cout << int(19.99) << std::endl; // both of them changes variable into integer
		std::cout << (int)19.99 << std::endl; // old style
		std::cout << int(cha4) << std::endl; // new style
		std::cout << (int)cha4 << std::endl;
	}
}