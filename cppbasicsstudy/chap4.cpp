...
namespace chap4 {
	...
	void chap4StudyPt2() {
		...
		*pn = 300;
		...
		// allocated memory by new can even be deleted at different function
		...
		// dynamic struct using new
		std::cout << "\n===== dynamic struct using new =====" << std::endl;
		inflatable* p_struct = new inflatable;
		std::cin.get(p_struct->name, 20);
		p_struct->price = 11.11; // when using new, there is no variable name; you can use -> instead
		(*p_struct).volume = 3.6; // using * also works, but have to use ()
		std::cout << (*p_struct).name << ' ' << (*p_struct).price << ' ' << p_struct->volume;
		delete p_struct;
	}
}